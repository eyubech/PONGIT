class NotFound extends HTMLElement {
	...
	async connectedCallback() {
		// Fetch HTML
		const htmlResponse = await fetch('../views/404-page.html');
		const htmlContent = await htmlResponse.text();
	
		// Fetch CSS
		const cssResponse = await fetch('../assets/css/404-page.css');
		const cssContent = await cssResponse.text();

		// Fetch JavaScript
		// const jsResponse = await fetch('../scripts/404-page.js');
  ...
  customElements.define('page-notfound-component', NotFound);
