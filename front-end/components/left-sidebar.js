...
class LeftSideBar extends HTMLElement {
	...
	async connectedCallback() {
		// Fetch HTML
		const htmlResponse = await fetch('../views/left-sidebar.html');
		const htmlContent = await htmlResponse.text();
	
		// Fetch CSS
		const cssResponse = await fetch('../assets/css/left-sidebar.css');
		const cssContent = await cssResponse.text();

		// Fetch JavaScript
		const jsResponse = await fetch('../scripts/left-sidebar.js');
	...
	select.forEach(ele => {
	  ...
	  if (li) {
			...
			{
				const nickname = li.parentElement.parentElement.previousElementSibling.children[0].children[2].textContent;
				const token = localStorage.getItem('accessToken');
				if (!token) {
	  ...
	  if (li) {
		  console.log("Delete button");
			const nickname = li.parentElement.parentElement.parentElement.children[0].children[0].children[2].textContent;
	...
	// build friend list
	function buildFriendList(nickname, imgPath, online_status)  {
		...
		<div class="profile">
			<a>
				<div class="status-indicator ${online_status}"></div>
				<img src="${imgPath}">
...
//   }
	...
	async function fetchFriends() {
		try {
		  ...
		  friendList.forEach((friend) => {
			...
			buildFriendList(friend.friend_username, imgPath, online_status);
		  });
			...
			li.addEventListener('click', async () => {
				...
				if (navActive === 'game') {
					...
					try {
						...
						const friendUsername = li.children[2].textContent;
					...
					// get the user data that am clicking on
					console.log("friend: ", friendUsername);
					window.loadComponent3(container, 'right-sidebar', user);
					let right_sidebar_bar = document.querySelector("right-sidebar-component")
					right_sidebar_bar.addEventListener("content-loaded", () => {
						...
						btnSendInviteGame.addEventListener("click", ( ) => {
							...
							socketNotify.send( JSON.stringify({
								"type"			:	"invite_game" ,
								"username"		:	liRightSide.dataset.username,
								"opponentId"	:	liRightSide.dataset.id
							}))
							// console.log("test button game ")
						})
						console.log("right bar is loaded\n\n\n\n\n")
					})
				}
			});
		});
	...
	async function handleAcceptRequest(li) {
		try {
			...
			// socketNotify.onmessage = ( event ) => {
			// 	const messageData = JSON.parse(event.data);
			// 	console.log("test message notifacation ")
			// }
