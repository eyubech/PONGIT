class MatchHistory extends HTMLElement {
	...
	async connectedCallback() {
		// Fetch HTML
		const htmlResponse = await fetch('../views/match-history.html');
		const htmlContent = await htmlResponse.text();
	
		// Fetch CSS
		const cssResponse = await fetch('../assets/css/match-history.css');
		const cssContent = await cssResponse.text();

		// Fetch JavaScript
		const jsResponse = await fetch('../scripts/match-history.js');
