class Notification extends HTMLElement {
	...
	async connectedCallback() {
		// Fetch HTML
		const htmlResponse = await fetch('../views/notification.html');
		const htmlContent = await htmlResponse.text();
	
		// Fetch CSS
		const cssResponse = await fetch('../assets/css/notification.css');
		const cssContent = await cssResponse.text();

		// Fetch JavaScript
		// const jsResponse = await fetch('../scripts/notification.js');
		...
		const tournamentModule = await import('../scripts/notification.js');
