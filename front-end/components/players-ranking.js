class PlayersRanking extends HTMLElement {
	...
	async connectedCallback() {
		// Fetch HTML
		const htmlResponse = await fetch('../views/players-ranking.html');
		const htmlContent = await htmlResponse.text();
	
		// Fetch CSS
		const cssResponse = await fetch('../assets/css/players-ranking.css');
		const cssContent = await cssResponse.text();

		// Fetch JavaScript
		const jsResponse = await fetch('../scripts/players-ranking.js');
  ...
  customElements.define('players-ranking-component', PlayersRanking);
